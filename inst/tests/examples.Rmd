...
Example using <code>expcoll</code>.  Unfortunately this interacts with the table of contents.
...
<hr>
### <smg>Figure:</smg> <fcap>Two <code>plotly</code> graphics combined into one</fcap>
<fcap>This used the <code>htmltools tagList</code> function.</fcap>
```{r plotlym}
...
<hr>
### `r cap('Proportions stratified by treatment')`
`r lcap('Second graph following has proportions (large symbols) and proportions stratified by treatment (small symbols)')`
```{r summaryM5}
