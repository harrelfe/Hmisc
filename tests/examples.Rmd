...
cap('Graphical output for <code>describe</code> function')
```
...
# Above: Proportions and chi-square test for categorical variables
```
```{r summaryM2}
plot(s, which='continuous', vars=1 : 4)
cap('Extended box plots for first 4 continuous predictors')
plot(s, which='continuous', vars=5 : 7)
cap('Extended box plots for remaining continuous predictors')
...
cap('Proportions (large symbols) and proportions stratified by treatment (small symbols)')
```
