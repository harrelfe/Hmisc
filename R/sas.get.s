...
csv.get <- function(file, lowernames=FALSE, datevars=NULL, datetimevars=NULL,
  ...
  ## See if data.table is available; if so use its faster fread
  w <- if(existsFunction('fread')) {
...
csv.get <- function(file, lowernames=FALSE, datevars=NULL, datetimevars=NULL,
  ...
  ## See if data.table is available; if so use its faster fread
  if(existsFunction('fread')) {
    if(comment.char != '')
      stop('comment.char not supported when data.table is in effect')
    w <- fread(file, header=! length(vnames), skip=skip, sep=sep, ...)
    if(length(vnames)) names(w) <- vnames
  }
  else {
    w <- if(length(vnames))
