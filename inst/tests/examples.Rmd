...
Example using <code>expcoll</code>.  Click on the down arrow to expand; it turns to an up arrow which can be clicked to contract.
`r mu$expcoll('Example table', html(data.frame(x1=runif(10), x2=letters[1:10]),file=FALSE))`
...
x <- htmltools::tagList(p)  #lapply(p, plotly::as.widget))
putHfig(x, 'This used the <code>htmltools tagList</code> function.',
...
putHfig(plot(s, which='continuous', vars=1 : 4),
        'Extended box plots for the first 4 continuous variables')
```

```{r summaryM3,results='asis'}
putHfig(plot(s, which='continuous', vars=5 : 7),
...
putHfig(p, "Spike histograms, means, quantiles, Gini's mean difference, and SD stratified by treatment",
...
putHfig(plot(s, groups='drug'),
        'Proportions stratified by treatment')
s <- summaryP(stage + sex + spiders ~ drug, data=pbcm)
putHfig(plot(s, marginVal='All', marginLabel='All Treatment Groups'),
