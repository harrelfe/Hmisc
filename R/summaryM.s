summaryM <- function(formula, data=NULL, subset, na.action=na.retain,
...
#  Y <- eval(mf, parent.frame())
   ...
   if(length(group)) {
    ...
    gname <- group.freq <- NULL
    glabel <- ''
  }

  quants <- unique(c(quant, 0.025, 0.05, 0.125, 0.25, 0.375, 0.5, 0.625,
                     0.75, 0.875, 0.95, 0.975))
    ...
    if(!inherits(w,'mChoice')) {
      ...
      if(is.factor(w) || is.logical(w)) {
        ...
        sfn <- function(x, quant) {
          ...
          on.exit(options(o))
          c(quantile(x,quant), Mean=mean(x), SD=sqrt(var(x)), N=sum(!is.na(x)))
        }
