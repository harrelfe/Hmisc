...
#' run through \code{fitter}, and a line plot is made instead of showing data points.
#'
...
#' @param fitter a fitting such as \code{loess}
...
plotlyM <- function(data, x=~x, y=~y, xhi=~xhi, yhi=~yhi, htext=NULL,
  ...
  if(! length(names(ylab))) stop('ylab elements must have names')
  ...
  for(vn in vlevs) {
    for(sn in stlevs) {
      ...
      j <- if(length(colvar)) order(w[[colvar]], w[[xn]])
             ...
             if(length(sizevar)) order(w[[sizevar]], w[[xn]])
           else order(w[[xn]])
      w <- w[j, ]
      r <- p
