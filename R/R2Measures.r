...
##'
##' Computes various generalized R^2 measures related to the Cox-Snell R^2 for regression models fitted with maximum likelihood.  The original Cox-Snell (also due to Maddala) R^2 is labeled `R2` in the result.  This measure uses the raw sample size `n` and does not penalize for overfitting, so it can be rewarded for overfitting.  A measure adjusted for the number of fitted regression coefficients `p` uses the analogy to R^2 in linear models by computing `1 - exp(- lr / n) * (n-1)/(n-p-1)` if `padj=2`, which is approximately `1 - exp(- (lr - p) / n)`, the version used if `padj=1` (the default).  The latter measure is appealing because the expected value of the likelihood ratio chi-square statistic `lr` is `p` under the global null hypothesis of no predictors being associated with the response variable.  See <https://hbiostat.org/bib/r2.html> for more details.
##'
##' It is well known that in logistic regression the Cox-Snell R^2 cannot achieve a value of 1.0 even with a perfect model, which prompted Nagelkerke to divide the R^2 measure by its maximum attainable value.  This is not necessarily the best recalibration of R^2 throughout its range.  An alternative is to use the formulas above but to replace the raw sample size `n` with the effective sample size, which for data with many ties can be significantly lower than the number of observations.  As used in the `popower()` and `describe()` functions, in the context of a Wilcoxon test or the proportional odds model, the effective sample size is `n * (1 - f)` where `f` is the sums of cubes of the proportion of observations at each distict value of the response variable.  To obtain R^2 measures using the effective sample size, either provide `ess` as a single number specifying the effective sample size, or specify a vector of frequencies of distinct Y values from which the effective sample size will be computed.  In the context of survival analysis, the single number effective sample size you may wish to specify is the number of uncensored observations.  This is exactly correct when estimating the hazard rate from a simple exponential distribution or when using the Cox PH/log-rank test.  For failure time distributions with a very high early hazard, censored observations contain enough information that the effective sample size is greater than the number of events.  See Benedetti et al, 1982.
##' @title R2Measures
...
##' @param padj set to 2 to use the classical adjusted R^2 penalty, 1 to subtract `p` from `lr`
##' @param inclval set to `TRUE` to R2 values in `html` and `latex` attributes, otherwise only the labels for the measures are returned
##' @return named vector of R2 measures, with two attributes: `html` contains the character string results formatted for html, and `latex` contains a character string vector of results formatted for LaTeX.  If `inclval=TRUE` these representations contain also the numeric results rounded to 3 decimal places.  The notation for results is R^2 sub `p, n` where the `p` component is empty for unadjusted estimtes and `n` is the sample size used (actual sample size for first measures, effective sample size for remaining ones).  The naming convention is for standard output to have `(p,n)` at the end of a name or `(p,n)` as subscripts in HTML and LaTeX output where `n` is the effective sample size and `p` is the number of regression parameters being adjusted for.  For indexes that are not adjusted, only `n` appears.
...
R2Measures <- function(lr, p, n, ess=NULL, padj=1, inclval=FALSE) {
  ...
  g <- function(r, p, n) {
    ...
    nam <- paste0('R2(', sub, ')')
    w <- if(inclval) paste0('=', round(r, 3)) else ''
    ltx <- paste0('$R^{2}_{', sub, '}', w, '$')
    html <- paste0('R<sup>2</sup><sub>', sub, '</sub>', w)
    return(list(name=nam, latex=ltx, html=html))
  }
  z    <- g(R, c(0, p), c(n, n))
  name <- z$name
  ltx  <- z$latex
