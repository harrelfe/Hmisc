...
#' @param center center point for making the two subsets
#'
...
#' pMedian(y) + 1.96 * c(- s2['bottom'], s2['top'])
#' c(mean=mean(y), pseudomedian=pMedian(y), median=median(y))

dualSD <- function(x, na.rm=FALSE, nmin=10, center=pMedian(x)) {
  ...
  s   <- function(x) sqrt(sum((x - center) ^ 2) / (length(x) - 1))
  a <- s(x[x <= center])
  b <- s(x[x >= center])
