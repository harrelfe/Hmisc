...
The `latexVerbatim` function is used to enclose output of `describe` in an html box with a smaller font (0.8 &times; the usual font size[^1]).  This uses an html `textarea`, i.e., a scrollable box.

[^1]: The default is 75% size.

```{r describe}
d <- describe(pbc)
htmlVerbatim(d, size=80, scroll=TRUE)
plot(d)
```
...
[^2]: `mu` is the part of the `Hmisc` package object `markupSpecs` that is for html.  It includes a function `session` that renders the session environment (including package versions) in html.
