...
#' @param type type of bootstrap interval, defaulting to `'percentile'`; may also be `'bca'`
#
#' @return a scalar numeric value if `conf.int = 0`, or a 3-vector otherwise, with named elements `estimate, lower, upper`.  If the number of non-missing values is less than 5, `NA` is returned for both lower and upper limits.
#' @export
#' @md
...
pMedian <- function(x, na.rm = FALSE, conf.int=0, B=1000, type=c('percentile', 'bca')) {
  ...
  l <- length(cl)
  c(estimate=res, lower=cl[l - 1], upper=cl[l])
}
